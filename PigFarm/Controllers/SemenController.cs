...
{
    public class SemenController : ApiControllerBase
    {
        private readonly ISemenService _service;

        public SemenController(ISemenService service)
        {
            _service = service;
        }

        [HttpGet]
        public async Task<ActionResult> GetSemens(string farmGuid)
        {
            return Ok(await _service.GetSemens(farmGuid));
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] SemenDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] SemenDto model)
        {
        ...
        [HttpGet]
        public async Task<ActionResult> GetSemens2()
        {
            ...
            return Ok(await _service.GetSemens(farmGuid, top, skip, search, selected));
        }
        [HttpGet]
