...
namespace PigFarm.Controllers
{
    ...
    public class LineNotifyController : ControllerBase
    {
        ...
        public LineNotifyController(IConfiguration config, ILineService service)
        {
            _config = config;
            _service = service;
        ...
        public async Task<ActionResult> SendMessage(MessageParams messageParams)
        {
            try
            {
                await _service.SendMessage(messageParams);
                return NoContent();
