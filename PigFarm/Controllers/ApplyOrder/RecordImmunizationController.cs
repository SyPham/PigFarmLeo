...
{
    public class RecordImmunizationController : ApiControllerBase
    {
        private readonly IRecordImmunizationService _service;

        public RecordImmunizationController(IRecordImmunizationService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] RecordImmunizationDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] RecordImmunizationDto model)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> LoadMobileData([FromBody] DataManager request, [FromQuery] string farmGuid, string lang, string penGuid, DateTime? estDate)
        {

            var data = await _service.LoadMobileData(request, farmGuid, lang, penGuid, estDate);
            return Ok(data);
        }
    }
}
