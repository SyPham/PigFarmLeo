...
{
    public class SemenMixController : ApiControllerBase
    {
        private readonly ISemenMixService _service;

        public SemenMixController(ISemenMixService service)
        {
            _service = service;
        }

        [HttpGet]
        public async Task<ActionResult> GetSemenMixes(string farmGuid)
        {
            return Ok(await _service.GetSemenMixes(farmGuid));
        }
        ...
        [HttpPost]
        public async Task<ActionResult> AddAsync([FromBody] SemenMixDto model)
        {
            return StatusCodeResult(await _service.AddAsync(model));
        }

        [HttpPut]
        public async Task<ActionResult> UpdateAsync([FromBody] SemenMixDto model)
        {
        ...
        [HttpGet]
        public async Task<ActionResult> GetSemenMixes2()
        {
            ...
            return Ok(await _service.GetSemenMixes(farmGuid, top, skip, search, selected));
        }
        [HttpGet]
