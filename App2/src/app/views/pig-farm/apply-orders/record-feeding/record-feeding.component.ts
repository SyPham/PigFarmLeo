...
import {
  RecordFeedingService ,Record2PigService,
...
@Component({
  selector: 'app-record-feeding',
  templateUrl: './record-feeding.component.html',
  styleUrls: ['./record-feeding.component.css']
})
export class RecordFeedingComponent
  ...
  // life cycle ejs-grid
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}RecordFeeding/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
