...
import { RecordCullingSaleService ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
import { RecordCullingSale } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-record-culling-sale',
  templateUrl: './record-culling-sale.component.html',
  styleUrls: ['./record-culling-sale.component.css']
})
export class RecordCullingSaleComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordCullingSale = new EventEmitter();
  data: DataManager;
  ...
  actionBeginPig(e:any) {
    ...
    if (e.action === 'edit' && e.requestType === 'save') {
      ...
      if (index !== -1) {
        ...
        const amounts = this.selectedPigData.map(x=> x.recordWeight);
  ...
  disabledInput(date : any) {
    ...
    constructor(
    private service: RecordCullingSaleService,
...
private router: Router,
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Culling_Sale")
      .toPromise();
  }
  async loadPen() {
 ...
 async loadPig() {
  ...
  loadSelectedPig() {
    ...
    this.servicePig.getSelectedPig3(p).subscribe((res: any) => {
      ...
      const weights = this.selectedPigData.map(x=> x.recordWeight);
        // this.selectedPig = this.selectedPigData.map(x=> x.pigGuid);
      const amounts = this.selectedPigData.map(x=> x.recordAmount);
      this.avgWeight = +this.average(weights).toFixed(0);
      this.totalWeight = +this.total(weights).toFixed(0);
      this.totalAmount = +this.total(amounts).toFixed(0);
    });
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_Culling_Sale");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordCullingSale(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_Culling_Sale");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordCullingSale/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  getCheckedPigData() {
    this.servicePig
      .getSelectedPigsByRecord(this.model?.guid, "Record_Culling_Sale")
      .subscribe((data: any) => {
  ...
  setStatus(model: RecordCullingSale) {
  ...
  onParamChange() {
    ...
    if (data?.id > 0) {
      ...
      try
        ...
        this.spinner.hide('default');
      this.title = "RECORD_CULLINGSALE_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "RECORD_CULLINGSALE_ADD_MODAL";
    }
