...
import {
  RecordRepairService ,Record2PigService,
...
@Component({
  selector: 'app-record-repair',
  templateUrl: './record-repair.component.html',
  styleUrls: ['./record-repair.component.css']
})
export class RecordRepairComponent
...
private router: Router,
  ...
  onChangeRoom(e) {
    ...
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Record_Repair");
  ...
  // life cycle ejs-grid
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Record_Repair");
