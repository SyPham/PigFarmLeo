...
import {
  RecordInventoryCheckService ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
import { RecordInventoryCheck } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-record-inventoryCheck',
  templateUrl: './record-inventoryCheck.component.html',
  styleUrls: ['./record-inventoryCheck.component.css']
})
export class RecordInventoryCheckComponent
  extends BaseComponent
  implements OnInit, OnDestroy
  {
    public query: Query;
    localLang = window.navigator.userLanguage || window.navigator.language;
    @Output() selectRecordInventoryCheck = new EventEmitter();
    data: DataManager;
...
public onFilteringPig: any = (e: any) => {
  ...
  disabledInput(date : any) {
    ...
    constructor(
      private service: RecordInventoryCheckService,
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Record_InventoryCheck")
        .toPromise();
    }
    loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_InventoryCheck");
  ...
  // life cycle ejs-grid
    ...
    recordClick(args: any) {
      ...
      this.service.changeRecordInventoryCheck(args.rowData);
    }
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Record_InventoryCheck");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}RecordInventoryCheck/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    setStatus(model: RecordInventoryCheck) {
    ...
    finance() {
      this.model.status = this.statusConts.Execute;
      this.model.financeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.financeDate = new Date();
      this.update();
    }
    execute() {
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.title = "RECORD_INVENTORYCHECK_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "RECORD_INVENTORYCHECK_ADD_MODAL";
      }
