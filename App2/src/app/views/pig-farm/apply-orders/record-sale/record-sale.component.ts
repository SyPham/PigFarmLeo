...
import { RecordSaleService   ,Record2PigService,
 ...
 async loadPig() {
  ...
  loadButtonConfig() {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_Sale");
        ...
        predicate = predicate.and("status", "equal", 1);
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_Sale");
  ...
  getCheckedPigData() {
    this.servicePig
      .getSelectedPigsByRecord(this.model?.guid, "Record_Sale")
      .subscribe((data: any) => {
