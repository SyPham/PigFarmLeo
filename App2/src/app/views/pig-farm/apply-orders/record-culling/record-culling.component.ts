...
import {
  RecordCullingService ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
import { Record2Pig, RecordCulling } from "src/app/_core/_model/apply-orders";
...
@Component({
  ...
  styleUrls: ['./record-culling.component.css'],
  providers: [SortService]
})
export class RecordCullingComponent
  ...
  configButtonData: any;
    ...
    selectedPenData: any[] = [];
    selectedPig: any[] = [];
    selectedPigData: Record2Pig[] = [];
  configData: any;
  oldselectedPigData: Record2Pig[];
    ...
    actionBeginPig(e:any) {
      ...
      if (e.action === 'edit' && e.requestType === 'save') {
        ...
        // // this.selectedPig = this.selectedPigData.map(x=> x.pigGuid);
          this.model.cullingWeight = +this.average(weights).toFixed(0);
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Record_Culling")
        .toPromise();
    }
   async loadPen() {
  ...
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_Culling");
  ...
  // life cycle ejs-grid
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Record_Culling");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}RecordCulling/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
