...
import { RecordTower } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { RecordTowerService   ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
declare let window: any;
@Component({
  selector: 'app-record-tower',
  templateUrl: './record-tower.component.html',
  styleUrls: ['./record-tower.component.css']
})
export class RecordTowerComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordTower = new EventEmitter();
  data: DataManager;
  ...
  disabledInput(date : any) {
    ...
    constructor(
    private service: RecordTowerService,
...
private router: Router,
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Tower")
      .subscribe((res: any) => {
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Tower")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Tower`,
      adaptor: new UrlAdaptor(),
  ...
  loadButtonConfig() {
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Record_Tower");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordTower(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_Tower");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordTower/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
...
this.getAudit(this.model.id);
      this.title = "RECORD_TOWER_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "RECORD_TOWER_ADD_MODAL";
    }
