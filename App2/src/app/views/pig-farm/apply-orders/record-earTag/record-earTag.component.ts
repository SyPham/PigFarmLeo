...
import {
  RecordEarTagService ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
import { RecordEarTag } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-record-earTag',
  templateUrl: './record-earTag.component.html',
  styleUrls: ['./record-earTag.component.css']
})
export class RecordEarTagComponent
  ...
  valueLabel: any;
    ...
    actionBeginPig(e:any) {
      ...
      if (e.action === 'edit' && e.requestType === 'save') {
        ...
        if (index !== -1) {
          this.selectedPigData[index].recordValue = e.data.recordValue;
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Record_EarTag")
        .toPromise();
    }
    async loadPen() {
  ...
  onChangeRoom(e) {
        ...
        for (const old of this.oldselectedPigData) {
          for (const news of this.selectedPigData) {
            if (old.pigGuid === news.pigGuid) {
              news.recordValue = old.recordValue;
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_EarTag");
  ...
  // life cycle ejs-grid
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Record_EarTag");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}RecordEarTag/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    async openModal(template, data = {} as any) {
      ...
      if (data?.id > 0) {
        ...
        this.spinner.hide('default');
        this.title = "RECORD_EARTAG_EDIT_MODAL";
      } else {
        this.model.id = 0;
        ...
        this.title = "RECORD_EARTAG_ADD_MODAL";
      }
