...
import { RecordInOut } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { RecordInOutService   ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
declare let window: any;
@Component({
  selector: 'app-record-inOut',
  templateUrl: './record-inOut.component.html',
  styleUrls: ['./record-inOut.component.css']
})
export class RecordInOutComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordInOut = new EventEmitter();
  data: DataManager;
...
private router: Router,
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_InOut")
      .subscribe((res: any) => {
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_InOut")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_InOut`,
      adaptor: new UrlAdaptor(),
  ...
  loadButtonConfig() {
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Record_InOut");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordInOut(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_InOut");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordInOut/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
...
this.getAudit(this.model.id);
      this.title = "RECORD_INOUT_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "RECORD_INOUT_ADD_MODAL";
    }
