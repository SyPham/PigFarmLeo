...
import { PurchaseService } from 'src/app/_core/_service/purchase.service';
...
loadLang() {
  this.translate.get('Purchase').subscribe( functionName => {
...
toolbarClick(args) {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Purchase/LoadData?farmGuid=${farmGuid}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Purchase).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Purchase) {
    if (data?.id > 0) {
      this.model = {...data};
