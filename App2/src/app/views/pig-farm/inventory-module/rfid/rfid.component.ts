...
import { Rfid } from 'src/app/_core/_model/inventories';
import { RfidService } from 'src/app/_core/_service/inventories/rfid.service';
...
export class RfidComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Rfid').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Rfid/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Rfid).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Rfid) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'RFID_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'RFID_ADD_MODAL';
    }
