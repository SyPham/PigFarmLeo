...
import { Semen } from 'src/app/_core/_model/inventories';
import { SemenService } from 'src/app/_core/_service/inventories/semen.service';
...
@Component({
  ...
  styleUrls: ['./semen.component.scss']
})
export class SemenComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Semen').subscribe( functionName => {
  ...
  locationChange(value) { this.model.location = value; }
  // life cycle ejs-grid
  dataBound() {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Semen/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Semen).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Semen) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'SEMEN_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'SEMEN_ADD_MODAL';
    }
