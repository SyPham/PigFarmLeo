...
import { SemenMix } from 'src/app/_core/_model/inventories';
import { SemenMixService } from 'src/app/_core/_service/inventories/semen-mix.service';

declare let window:any;

@Component({
  selector: 'app-semen-mix',
  templateUrl: './semen-mix.component.html',
  styleUrls: ['./semen-mix.component.scss']
})
export class SemenMixComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    private service: SemenMixService,
  ...
  loadLang() {
    this.translate.get('SemenMix').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}SemenMix/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as SemenMix).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as SemenMix) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'SEMEN_MIX_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'SEMEN_MIX_ADD_MODAL';
    }
