...
import {
  RecordCullingService,
} from "src/app/_core/_service/apply-orders";
import { RecordCulling } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-exe-record-culling',
  templateUrl: './exe-record-culling.component.html',
  styleUrls: ['./exe-record-culling.component.css']
})
export class ExeRecordCullingComponent
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Culling")
      .subscribe((res: any) => {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Culling`,
      adaptor: new UrlAdaptor(),
