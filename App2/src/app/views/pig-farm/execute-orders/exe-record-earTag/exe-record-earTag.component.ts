...
@Component({
  selector: 'app-exe-record-earTag',
  templateUrl: './exe-record-earTag.component.html',
  styleUrls: ['./exe-record-earTag.component.css']
})
export class ExeRecordEarTagComponent
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_EarTag")
      .subscribe((res: any) => {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_EarTag`,
      adaptor: new UrlAdaptor(),
