...
import {
  RecordImmunizationService,
} from "src/app/_core/_service/apply-orders";
import { RecordImmunization } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-exe-record-immunization',
  templateUrl: './exe-record-immunization.component.html',
  styleUrls: ['./exe-record-immunization.component.css']
})
export class ExeRecordImmunizationComponent
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Immunization")
      .subscribe((res: any) => {
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Immunization")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Immunization`,
      adaptor: new UrlAdaptor(),
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordImmunization/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
      this.query
  ...
  setStatus(model: RecordImmunization) {
