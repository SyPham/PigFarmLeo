...
import { RecordDeath } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-exe-record-death',
  templateUrl: './exe-record-death.component.html',
  styleUrls: ['./exe-record-death.component.css']
})
export class ExeRecordDeathComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordDeath = new EventEmitter();
  data: DataManager;
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Death")
      .subscribe((res: any) => {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Death`,
      adaptor: new UrlAdaptor(),
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordDeath(args.rowData);
  }
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordDeath/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
      this.query
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = "RECORD_DEATH_EDIT_MODAL";
    } else {
      this.model.id = 0;
      this.model.makeOrderGuid = this.makeOrderGuid;
      this.model.status = this.statusConts.Close;
      ...
      this.title = "RECORD_DEATH_ADD_MODAL";
    }
