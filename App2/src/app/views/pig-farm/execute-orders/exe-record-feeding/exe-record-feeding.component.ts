...
import {
  Record2PigService,
  RecordFeedingService,
} from "src/app/_core/_service/apply-orders";
import { RecordFeeding } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-exe-record-feeding',
  templateUrl: './exe-record-feeding.component.html',
  styleUrls: ['./exe-record-feeding.component.css']
})
export class ExeRecordFeedingComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordFeeding = new EventEmitter();
  data: DataManager;
...
enabledInput(date : any) {
  ...
  // getDate
  let date2 = ""
  if (new Date() && this.getDate instanceof Date) {
...
pigGuid: string;
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Feeding")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    this.queryPig = new Query()
    .skip(0)
    .take(1000)
    .where("farmGuid", "equal", localStorage.getItem("farmGuid"))
    .where("status", "equal", 1);
    if (this.selectedPen.length > 0) {
      const pen = this.selectedPen[0];
      let predicate: Predicate = new Predicate('penGuid', 'equal', pen);
      for (const item of this.selectedPen ) {
        if (pen != item) {
          predicate = predicate.or('penGuid', 'equal', item);
        }

      }
      this.queryPig.where(predicate);
    } else {
      this.queryPig.where("roomGuid", "equal", this.roomGuid);
    }




    this.dataPig = new DataManager(
      {
        url: `${this.baseUrl}Pig/GetDataDropdownlist`,
        adaptor: new UrlAdaptor(),
        crossDomain: true,
      },
      this.queryPig.sortBy('earNo')
    );
  }
    onChangeMakeOrder(e) {
  if (e.isInteracted) {
...
onChangeRoom(e) {
  ...
  let predicate: Predicate = new Predicate("type", "equal", "Record_Sale");
...
// life cycle ejs-grid
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordFeeding(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_Feeding");
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordFeeding/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}&status=${this.statusConts.Execute}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: RecordFeeding) {
    if (model.agreeGuid && !model.executeGuid) {
      this.model.status = this.statusConts.Agree;
    } else if (model.executeGuid) {
      this.model.status =this.model.id > 0 ? this.statusConts.Execute:  this.statusConts.Close;
  ...
  execute() {
    ...
    if (this.model.id > 0) {
      ...
      this.model.applyDate = null;
      this.model.applyGuid = null;
      this.update();
    } else {
      this.model.status = this.statusConts.Close;
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = "RECORD_FEEDING_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "RECORD_FEEDING_ADD_MODAL";
    }
