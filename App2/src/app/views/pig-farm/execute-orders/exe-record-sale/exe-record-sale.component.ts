...
import { RecordSale } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: 'app-exe-record-sale',
  templateUrl: './exe-record-sale.component.html',
  styleUrls: ['./exe-record-sale.component.css']
})
export class ExeRecordSaleComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordSale = new EventEmitter();
  data: DataManager;
  ...
  constructor(
    private service: RecordSaleService,
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Sale")
      .subscribe((res: any) => {
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_Sale")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Sale`,
      adaptor: new UrlAdaptor(),
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordSale(args.rowData);
  }
  ...
  loadData() {
    ...
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordSale/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
      this.query
  ...
  async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = "RECORD_SALE_EDIT_MODAL";
    } else {
      this.model.id = 0;
      this.model.makeOrderGuid = this.makeOrderGuid;
      this.model.status = this.statusConts.Close;
      ...
      this.title = "RECORD_SALE_ADD_MODAL";
    }
