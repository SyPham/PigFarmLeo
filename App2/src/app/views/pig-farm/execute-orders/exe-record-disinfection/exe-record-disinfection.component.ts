...
import { RecordDisinfection } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { Record2PigService, RecordDisinfectionService } from "src/app/_core/_service/apply-orders";
declare let window: any;
@Component({
  selector: 'app-exe-record-disinfection',
  templateUrl: './exe-record-disinfection.component.html',
  styleUrls: ['./exe-record-disinfection.component.css']
})
export class ExeRecordDisinfectionComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordDisinfection = new EventEmitter();
  data: DataManager;
...
disabledInput(date : any) {
  ...
  constructor(
    private service: RecordDisinfectionService,
...
pigGuid: string;
  ...
  loadButtonConfig() {
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Record_Disinfection");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordDisinfection(args.rowData);
  }
  ...
  loadData() {
    ...
    this.query.where(new Predicate("status", "equal", this.orderStatus.Execute));
    const accessToken = localStorage.getItem("token");
    const farmGuid = localStorage.getItem("farmGuid");
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordDisinfection/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  setStatus(model: RecordDisinfection) {
...
this.getAudit(this.model.id);
      this.title = "RECORD_DISINFECTION_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.title = "RECORD_DISINFECTION_ADD_MODAL";
    }
