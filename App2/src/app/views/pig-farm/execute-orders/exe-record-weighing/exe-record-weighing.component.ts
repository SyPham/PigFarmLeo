...
import {
  Record2PigService,
  RecordWeighingService,
} from "src/app/_core/_service/apply-orders";
import { RecordWeighing } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
import { NgxSpinnerService } from "ngx-spinner";
declare let window: any;
@Component({
  selector: 'app-exe-record-weighing',
  templateUrl: './exe-record-weighing.component.html',
  styleUrls: ['./exe-record-weighing.component.css']
})
export class ExeRecordWeighingComponent
  ...
  oldselectedPigData: any[];
    ...
    actionBeginPig(e:any) {
      ...
      if (e.action === 'edit' && e.requestType === 'save') {
        ...
        // this.selectedPig = this.selectedPigData.map(x=> x.pigGuid);
          this.model.netWeight = +this.average(weights).toFixed(0);
    ...
    avgWeightChange(e) {
      this.model.netWeight = this.avgWeight;
    }
...
private router: Router,
  ...
  estDate: any;
    ...
    loadRoomGuid() {
      return this.serviceRoom
        .getRoomByRecord(this.model?.guid || "", "Record_Weighing")
        .toPromise();
    }
    async loadPen() {
   ...
   async loadPig() {
    ...
    if (e.isInteracted) {
      ...
      this.model.makeOrderGuid = e.itemData.guid;
    }
  }
  onChangeRoom(e) {
    ...
    let predicate: Predicate = new Predicate("type", "equal", "Record_Weighing");
  ...
  // life cycle ejs-grid
    ...
    loadConfig() {
      let query = new Query();
      query.where("type", "equal", "Record_Weighing");
    ...
    loadData() {
      ...
      this.data = new DataManager(
        {
          url: `${this.baseUrl}RecordWeighing/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
          adaptor: new UrlAdaptor(),
          headers: [{ authorization: `Bearer ${accessToken}` }],
        },
    ...
    setStatus(model: RecordWeighing) {
