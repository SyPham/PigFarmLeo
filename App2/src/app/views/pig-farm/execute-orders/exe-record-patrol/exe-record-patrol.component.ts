...
@Component({
  selector: 'app-exe-record-patrol',
  templateUrl: './exe-record-patrol.component.html',
  styleUrls: ['./exe-record-patrol.component.css']
})
export class ExeRecordPatrolComponent
...
pigGuid: string;
  ...
  loadPenGuid() {
    this.servicePen
      .getPenByRecord(this.model?.guid || "", "Record_Patrol")
      .subscribe((res: any) => {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_Patrol`,
      adaptor: new UrlAdaptor(),
