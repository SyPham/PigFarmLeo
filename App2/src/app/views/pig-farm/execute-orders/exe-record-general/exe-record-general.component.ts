...
import {
  RecordGeneralService ,Record2PigService,
} from "src/app/_core/_service/apply-orders";
import { RecordGeneral } from "src/app/_core/_model/apply-orders";
import { PenService, RoomService } from "src/app/_core/_service/farms";
declare let window: any;
@Component({
  selector: "app-exe-record-general",
  templateUrl: "./exe-record-general.component.html",
  styleUrls: ["./exe-record-general.component.css"],
})
export class ExeRecordGeneralComponent
  extends BaseComponent
  implements OnInit, OnDestroy
{
  public query: Query;
  localLang = window.navigator.userLanguage || window.navigator.language;
  @Output() selectRecordGeneral = new EventEmitter();
  data: DataManager;
  ...
  disabledInput(date : any) {
    ...
    constructor(
    private service: RecordGeneralService,
...
private router: Router,
  ...
  loadPenGuid(recordGeneralGuid) {
    this.servicePen
      .getPenByRecord(recordGeneralGuid || "", "Record_General")
      .subscribe((res: any) => {
  ...
  loadRoomGuid() {
    return this.serviceRoom
      .getRoomByRecord(this.model?.guid || "", "Record_General")
      .toPromise();
  }
  loadPen() {
  ...
  loadPig() {
    ...
    this.pigData = new DataManager({
      url: `${this.baseUrl}Pig/GetPigsByPen?penGuid=${
        this.penGuid || ""
      }&recordGuid=${this.model?.guid || ""}&type=Record_General`,
  ...
  loadButtonConfig() {
    let query = new Query();
    const accessToken = localStorage.getItem("token");
    let predicate: Predicate = new Predicate("type", "equal", "Record_General");
  ...
  recordClick(args: any) {
    ...
    this.service.changeRecordGeneral(args.rowData);
  }
  ...
  loadConfig() {
    let query = new Query();
    query.where("type", "equal", "Record_General");
  ...
  loadData() {
    ...
    this.query.where("status", "equal", this.statusConts.Execute);

    const accessToken = localStorage.getItem("token");
    const farmGuid = localStorage.getItem("farmGuid");
    this.data = new DataManager(
      {
        url: `${this.baseUrl}RecordGeneral/LoadData?farmGuid=${farmGuid}&makeOrderGuid=&lang=${this.globalLang}`,
        adaptor: new UrlAdaptor(),
        headers: [{ authorization: `Bearer ${accessToken}` }],
      },
  ...
  onParamChange() {
    ...
    if (data?.id > 0) {
      ...
      this.loadPen();
      this.loadPenGuid(this.model.guid);
this.getAudit(this.model.id);
      this.title = "RECORD_GENERAL_EDIT_MODAL";
    } else {
      this.model.id = 0;
      ...
      this.model.executeGuid = JSON.parse(localStorage.getItem("user")).guid;
      this.model.executeDate = new Date();
      this.title = "RECORD_GENERAL_ADD_MODAL";
    }
