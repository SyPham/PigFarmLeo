...
import { Room, FarmScreen, BarnScreen } from 'src/app/_core/_model/farms';
import { AreaService, RoomService, FarmService, BarnService } from 'src/app/_core/_service/farms';
...
export class RoomComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.push(this.serviceBarn.currentBarn.subscribe(barn => {
      ...
      // if (this.area?.guid && this.farm?.guid && this.barn?.guid) {
      //   this.loadData();
      // }
    }));
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Room/LoadData?farmGuid=${this.farm?.guid || ''}&areaGuid=${this.area?.guid || ''}&barnGuid=${this.barn?.guid || ''}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.model.startDate = this.ToDate(this.model.startDate);
        this.model.endDate = this.ToDate(this.model.endDate);
        this.service.update(this.model as Room).subscribe(
          (res) => {
            if (res.success === true) {
