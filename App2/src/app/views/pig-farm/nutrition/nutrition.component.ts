...
import { Nutrition } from 'src/app/_core/_model/nutrition';
import { NutritionService } from 'src/app/_core/_service/nutrition.service';
...
export class NutritionComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Nutrition').subscribe( functionName => {
  ...
  typeChange(value) { this.model.nutritionType = value; }
  // life cycle ejs-grid
  toolbarClick(args) {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Nutrition/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Nutrition).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Nutrition) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'NUTRITION_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'NUTRITION_ADD_MODAL';
    }
