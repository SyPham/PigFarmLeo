...
@Component({
  selector: 'app-record-pig-ear',
  templateUrl: './record-pig-ear.component.html',
  styleUrls: ['./record-pig-ear.component.scss']
})
export class RecordPigEarComponent extends BaseComponent implements OnInit,OnDestroy, IBatchWorkCommon {
 ...
 valueChange(value) {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}RecordEarTag/BatchWorkLoadData?farmGuid=${farmGuid}&lang=${this.globalLang}&${query}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'RECORD_EAR_TAG_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.makeOrderGuid = this.makeOrderGuid;
      this.model.recordDate = new Date();
      this.title = 'RECORD_EAR_TAG_ADD_MODAL';
    }
