...
import { InventoryScrap, Inventory } from 'src/app/_core/_model/inventories';
import { InventoryService, InventoryScrapService } from 'src/app/_core/_service/inventories';
import { Subscription } from 'rxjs';
@Component({
  selector: 'app-inventory-scrap',
  templateUrl: './inventory-scrap.component.html',
  styleUrls: ['./inventory-scrap.component.scss']
})
export class InventoryScrapComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: InventoryScrapService,
  ...
  ngOnChanges(scraps: SimpleChanges): void {

    if (this.bottomHeight != scraps.bottomHeight?.currentValue) {
      this.bottomHeight = scraps.bottomHeight?.currentValue;
    }
  }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}InventoryScrap/LoadData?farmGuid=${farmGuid}&fromInventoryGuid=${this.inventory?.guid}`,
