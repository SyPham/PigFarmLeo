...
import { AcceptanceInspection, Acceptance } from 'src/app/_core/_model/acceptances';
import { AcceptanceService, AcceptanceInspectionService } from 'src/app/_core/_service/acceptances';
import { Subscription } from 'rxjs';
import { DatePipe } from '@angular/common';
import { XAccountService } from 'src/app/_core/_service/xaccount.service';
@Component({
  selector: 'app-acceptance-inspection',
  templateUrl: './acceptance-inspection.component.html',
  styleUrls: ['./acceptance-inspection.component.scss']
})
export class AcceptanceInspectionComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: AcceptanceInspectionService,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.acceptance != changes.acceptance?.currentValue) {
      this.acceptance = changes.acceptance?.currentValue;
      if (this.acceptance) {
        this.loadData();
      }
    }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}AcceptanceInspection/LoadData?acceptanceGuid=${this.acceptance?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'ACCEPTANCE_INSPECTION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'ACCEPTANCE_INSPECTION_ADD_MODAL';
    }
