...
import { PigIsolation, PigDisease } from 'src/app/_core/_model/pig-disease';
import { PigDiseaseService, PigIsolationService } from 'src/app/_core/_service/pig-disease';
import { Subscription } from 'rxjs';
import { DatePipe } from '@angular/common';
@Component({
  selector: 'app-pig-isolation',
  templateUrl: './pig-isolation.component.html',
  styleUrls: ['./pig-isolation.component.scss']
})
export class PigIsolationComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: PigIsolationService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigIsolation/LoadData?upperGuid=${this.pigDisease?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_ISOLATION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'PIG_ISOLATION_ADD_MODAL';
    }
