...
import { PigDiagnosis, PigDisease } from 'src/app/_core/_model/pig-disease';
import { PigDiseaseService, PigDiagnosisService } from 'src/app/_core/_service/pig-disease';
import { Subscription } from 'rxjs';
import { DatePipe } from '@angular/common';
@Component({
  selector: 'app-pig-diagnosis',
  templateUrl: './pig-diagnosis.component.html',
  styleUrls: ['./pig-diagnosis.component.scss']
})
export class PigDiagnosisComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: PigDiagnosisService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigDiagnosis/LoadData?upperGuid=${this.pigDisease?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_DIAGNOSIS_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'PIG_DIAGNOSIS_ADD_MODAL';
    }
