...
import { PigPrescription, PigDisease } from 'src/app/_core/_model/pig-disease';
import { PigDiseaseService, PigPrescriptionService } from 'src/app/_core/_service/pig-disease';
import { Subscription } from 'rxjs';
import { DatePipe } from '@angular/common';
@Component({
  selector: 'app-pig-prescription',
  templateUrl: './pig-prescription.component.html',
  styleUrls: ['./pig-prescription.component.scss']
})
export class PigPrescriptionComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: PigPrescriptionService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigPrescription/LoadData?upperGuid=${this.pigDisease?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_PRESCRIPTION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'PIG_PRESCRIPTION_ADD_MODAL';
    }
