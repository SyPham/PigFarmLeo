...
import { PigCulling, PigDisease } from 'src/app/_core/_model/pig-disease';
import { PigDiseaseService, PigCullingService } from 'src/app/_core/_service/pig-disease';
import { Subscription } from 'rxjs';
import { DatePipe } from '@angular/common';
@Component({
  selector: 'app-pig-culling',
  templateUrl: './pig-culling.component.html',
  styleUrls: ['./pig-culling.component.scss']
})
export class PigCullingComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: PigCullingService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigCulling/LoadData?upperGuid=${this.pigDisease?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_CULLING_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'PIG_CULLING_ADD_MODAL';
    }
