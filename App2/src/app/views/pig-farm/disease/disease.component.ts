...
import { Disease } from 'src/app/_core/_model/disease';
import { DiseaseService } from 'src/app/_core/_service/disease.service';
...
export class DiseaseComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Disease').subscribe( functionName => {
  ...
  typeChange(value) { this.model.diseaseType = value; }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Disease/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Disease).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Disease) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'DISEASE_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'DISEASE_ADD_MODAL';
    }
