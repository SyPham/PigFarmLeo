...
import { REPAIR_TAB_Constant, PIG_SETTING_TAB_Constant } from 'src/app/_core/_constants';
...
import { RepairService } from 'src/app/_core/_service/repairs';
import { Repair } from 'src/app/_core/_model/repairs';
...
onResize(event) {
  ...
  ngOnInit() {
    this.active = REPAIR_TAB_Constant.Record;
  ...
  recordClick(args: any) {
    ...
    this.repair = args.rowData;
    this.service.changeRepair(this.repair);
 }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Repair/LoadData?farmGuid=${farmGuid}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'REPAIR_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = farmGuid;
      this.title = 'REPAIR_ADD_MODAL';
    }
