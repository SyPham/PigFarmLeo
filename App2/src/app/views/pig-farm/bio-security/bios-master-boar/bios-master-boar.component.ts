...
@Component({
  selector: 'app-bios-master-boar',
  templateUrl: './bios-master-boar.component.html',
  styleUrls: ['./bios-master-boar.component.scss']
})
export class BiosMasterBoarComponent extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Boar;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Boar;
