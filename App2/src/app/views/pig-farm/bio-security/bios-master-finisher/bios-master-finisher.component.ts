...
@Component({
  selector: 'app-bios-master-finisher',
  templateUrl: './bios-master-finisher.component.html',
  styleUrls: ['./bios-master-finisher.component.scss']
})
export class BiosMasterFinisherComponent  extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Finisher;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Finisher;
