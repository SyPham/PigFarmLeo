...
@Component({
  selector: 'app-bios-master-suckling',
  templateUrl: './bios-master-suckling.component.html',
  styleUrls: ['./bios-master-suckling.component.scss']
})
export class BiosMasterSucklingComponent extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Suckling;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Suckling;
