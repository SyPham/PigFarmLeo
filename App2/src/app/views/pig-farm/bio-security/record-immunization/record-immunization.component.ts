...
import { BioSRecord, BioSMaster, RecordImmunization } from 'src/app/_core/_model/bios';
import { BioSMasterService, BioSRecordService } from 'src/app/_core/_service/bios';
import { Subscription } from 'rxjs';
import { RecordImmunizationService } from 'src/app/_core/_service/bios/record-immunization.service';
@Component({
  selector: 'app-record-immunization-bios',
  templateUrl: './record-immunization.component.html',
  styleUrls: ['./record-immunization.component.scss']
})
export class RecordImmunizationComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}RecordImmunization/LoadData?bioSMasterGuid=${this.bioSMaster?.guid}&lang=${this.globalLang}`,
  ...
  create() {
   ...
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.service.add(this.model as any).subscribe(
          (res) => {
            if (res.success === true) {
