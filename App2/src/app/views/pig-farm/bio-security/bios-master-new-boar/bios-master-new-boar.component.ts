...
@Component({
  selector: 'app-bios-master-new-boar',
  templateUrl: './bios-master-new-boar.component.html',
  styleUrls: ['./bios-master-new-boar.component.scss']
})
export class BiosMasterNewBoarComponent extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.NewBoar;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.NewBoar;
