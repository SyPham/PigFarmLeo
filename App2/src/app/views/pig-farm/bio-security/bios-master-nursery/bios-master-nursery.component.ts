...
@Component({
  selector: 'app-bios-master-nursery',
  templateUrl: './bios-master-nursery.component.html',
  styleUrls: ['./bios-master-nursery.component.scss']
})
export class BiosMasterNurseryComponent extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Nursery;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Nursery;
