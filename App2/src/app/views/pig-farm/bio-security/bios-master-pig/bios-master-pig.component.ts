...
@Component({
  selector: 'app-bios-master-pig',
  templateUrl: './bios-master-pig.component.html',
  styleUrls: ['./bios-master-pig.component.scss']
})
export class BiosMasterPigComponent extends BaseComponent implements OnInit,OnChanges {
  ...
  loadData() {
    ...
    const pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Pig;
 ...
 async openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.model.id = 0;
      this.model.pigType = BIO_SECURITY_MASTER_PIG_TYPE_Constant.Pig;
