import { PIG_FARM_VECTOR_TAB_Constant, PIG_SETTING_TAB_Constant } from 'src/app/_core/_constants';
...
@Component({
  selector: 'app-pig-farm-vector',
  templateUrl: './pig-farm-vector.component.html',
  styleUrls: ['./pig-farm-vector.component.scss']
})
export class PigFarmVectorComponent extends BaseComponent implements OnInit {
...
onResize(event) {
  ...
  ngOnInit() {
    this.active = PIG_FARM_VECTOR_TAB_Constant.Plan;
  ...
  rowSelected(args) {
    ...
    if (args.data) {
      this.service.changePigFarmVector(args.data);
    }
  }
  recordClick(args: any) {
    //console.log(args.rowData);
    this.pigFarmVector = args.rowData;
    this.service.changePigFarmVector(this.pigFarmVector);
 }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigFarmVector/LoadData?lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    if (data?.id > 0) {
      ...
      this.title = 'PIG_FARM_VECTOR_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.title = 'PIG_FARM_VECTOR_ADD_MODAL';
    }
