...
@Component({
  selector: 'app-pig-farm-vectorcontrol',
  templateUrl: './pig-farm-vectorcontrol.component.html',
  styleUrls: ['./pig-farm-vectorcontrol.component.scss']
})
export class PigFarmVectorcontrolComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_FARM_VECTOR_CONTROL_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'PIG_FARM_VECTOR_CONTROL_ADD_MODAL';
    }
