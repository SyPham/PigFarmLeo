...
import { Material } from 'src/app/_core/_model/material';
...
export class MaterialComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Material').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Material/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as Material) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'MATERIAL_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'MATERIAL_ADD_MODAL';
    }
