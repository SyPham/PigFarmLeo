...
import { Disinfection } from 'src/app/_core/_model/disinfection';
import { DisinfectionService } from 'src/app/_core/_service/disinfection.service';
...
@Component({
  selector: 'app-disinfection2',
  templateUrl: './disinfection.component.html',
  styleUrls: ['./disinfection.component.scss']
})
export class Disinfection2Component extends BaseComponent implements OnInit {
  ...
  vendorData: any = [];
  constructor(
    private service: DisinfectionService,
  ...
  loadLang() {
    this.translate.get('Disinfection').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Disinfection/LoadData?farmGuid=${farmGuid}&lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.service.update(this.model as Disinfection).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Disinfection) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'DISINFECTION_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'DISINFECTION_ADD_MODAL';
    }
