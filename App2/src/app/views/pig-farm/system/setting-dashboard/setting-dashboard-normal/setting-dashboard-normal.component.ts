...
@Component({
  selector: 'app-setting-dashboard-normal',
  templateUrl: './setting-dashboard-normal.component.html',
  styleUrls: ['./setting-dashboard-normal.component.scss']
})
export class SettingDashboardNormalComponent extends BaseComponent implements OnInit, OnChanges, OnDestroy {
...
public commonFn: (args: { [key: string]: string }) => boolean = (args: { [key: string]: string }) => {
  ...
  actionBegin(args) {
    ...
    if (args.requestType === 'beginEdit') {
      ...
      this.type = data.type || '';
      this.textColor01 = data.textColor01 || '#ffffff';
...
toolbarClick(args) {
  ...
  loadData() {
    ...
    let query = `&type=A&type=B&type=C&type=D`;
