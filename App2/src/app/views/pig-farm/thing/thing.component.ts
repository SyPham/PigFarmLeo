...
export class ThingComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Thing').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Thing/LoadData?farmGuid=${farmGuid}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.service.update(this.model as Thing).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Thing) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'THING_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'THING_ADD_MODAL';
    }
