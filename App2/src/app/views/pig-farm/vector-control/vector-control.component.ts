...
import { VectorControl } from 'src/app/_core/_model/vector-control';
import { VectorControlService } from 'src/app/_core/_service/vector-control.service';
...
@Component({
  selector: 'app-vector-control2',
  templateUrl: './vector-control.component.html',
  styleUrls: ['./vector-control.component.css']
})
export class VectorControl2Component extends BaseComponent implements OnInit {
  ...
  vendorData: any = [];
  constructor(
    private service: VectorControlService,
  ...
  loadLang() {
    this.translate.get('VectorControl').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}VectorControl/LoadData?lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.service.update(this.model as VectorControl).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as VectorControl) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'VECTOR_CONTROL_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'VECTOR_CONTROL_ADD_MODAL';
    }
