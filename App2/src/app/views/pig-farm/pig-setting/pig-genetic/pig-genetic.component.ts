...
import { PigGeneticService } from 'src/app/_core/_service/pigs';
import { PigGenetic, PigPedigree } from 'src/app/_core/_model/pigs';
import { environment } from 'src/environments/environment';
import { DatePipe } from '@angular/common';
declare let window:any;
@Component({
  selector: 'app-pig-genetic',
  templateUrl: './pig-genetic.component.html',
  styleUrls: ['./pig-genetic.component.scss']
})
export class PigGeneticComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: PigGeneticService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}PigGenetic/LoadData?pigPedigreeGuid=${this.pigPedigree?.guid}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'PIG_GENETIC_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.pedigreeGuid = this.pigPedigree.guid;
      this.title = 'PIG_GENETIC_ADD_MODAL';
    }
