...
import { BOMDisinfection } from 'src/app/_core/_model/bom-disinfection';
import { BOMDisinfectionService } from 'src/app/_core/_service/bom-disinfection.service';
...
@Component({
  ...
  styleUrls: ['./disinfection.component.css']
})
export class DisinfectionComponent extends BaseComponent implements OnInit {
  @Input() bom: Bom;
  data: DataManager;
  ...
  constructor(
    private service: BOMDisinfectionService,
  ...
  typeChange(value) { this.model.disinfectionType = value; }
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BOMDisinfection/LoadDataByBOMGuid?bomGuid=${this.bom?.guid}&lang=${this.globalLang}`,
  ...
  create() {
   ...
   this.alertify.confirm4(
      ...
      () => {
        ...
        this.service.add(this.model as BOMDisinfection).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  update() {
   ...
   this.alertify.confirm4(
      ...
      () => {
        this.model.bomGuid = this.bom?.guid;
        this.service.update(this.model as BOMDisinfection).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'BOM_DISINFECTION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'BOM_DISINFECTION_ADD_MODAL';
    }
