...
export class BomLeftComponent extends BaseComponent implements OnInit {
  @Output() selectBOM = new EventEmitter();
  data: DataManager;
  password = '';
  ...
  ngOnInit() {
    ...
    L10n.load(load);
    this.serviceShare.changeBOM({} as Bom);
    this.loadData();
  }
  ...
  rowSelected(args) {
    if (args.data) {
      this.serviceShare.changeBOM(args.data);
    }
  }
  recordClick(args: any) {
    //console.log(args.rowData);
    this.serviceShare.changeBOM(args.rowData);
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BOM/LoadData?lang=${this.globalLang}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Bom).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Bom) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BOM_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.title = 'BOM_ADD_MODAL';
    }
