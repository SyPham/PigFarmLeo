...
export class ImmunizationComponent extends BaseComponent implements OnInit {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}BOMImmunization/LoadDataByBOMGuid?farmGuid=${localStorage.getItem('farmGuid')}&bomGuid=${this.bom?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    if (!this.bom?.guid) {
      this.alertify.warning(this.translate.instant('Please choose the BOM!'), true);
      return;
    }
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'BOM_IMMUNIZATION_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'BOM_IMMUNIZATION_ADD_MODAL';
    }
