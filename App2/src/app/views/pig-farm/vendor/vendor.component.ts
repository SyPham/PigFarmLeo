...
import { Vendor } from 'src/app/_core/_model/vendor';
...
export class VendorComponent extends BaseComponent implements OnInit {
  ...
  loadLang() {
    this.translate.get('Vendor').subscribe( functionName => {
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}Vendor/LoadData?farmGuid=${farmGuid}`,
  ...
  update() {
   this.alertify.confirm4(
      ...
      () => {
        this.service.update(this.model as Vendor).subscribe(
          (res) => {
            if (res.success === true) {
  ...
  openModal(template, data = {} as Vendor) {
    if (data?.id > 0) {
      this.model = {...data};
      this.getAudit(this.model.id);
      this.title = 'VENDOR_EDIT_MODAL';
    } else {
      this.model.id = 0;
      this.model.farmGuid = localStorage.getItem('farmGuid');
      this.title = 'VENDOR_ADD_MODAL';
    }
