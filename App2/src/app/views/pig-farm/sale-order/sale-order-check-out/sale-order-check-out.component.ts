...
import { SalesOrderCheckOut, SalesOrder } from 'src/app/_core/_model/sale-orders';
import { SalesOrderService, SalesOrderCheckOutService } from 'src/app/_core/_service/sale-orders';
import { Subscription } from 'rxjs';
import { XAccountService } from 'src/app/_core/_service/xaccount.service';

@Component({
  selector: 'app-sale-order-check-out',
  templateUrl: './sale-order-check-out.component.html',
  styleUrls: ['./sale-order-check-out.component.scss']
})
export class SaleOrderCheckOutComponent extends BaseComponent implements OnInit, OnChanges {
  ...
  constructor(
    private service: SalesOrderCheckOutService,
  ...
  loadData() {
    ...
    this.data = new DataManager({
      url: `${this.baseUrl}SalesOrderCheckOut/LoadData?salesOrderGuid=${this.salesOrder?.guid}&lang=${this.globalLang}`,
  ...
  openModal(template, data = {} as any) {
    ...
    if (data?.id > 0) {
      ...
      this.title = 'SALE_ORDER_CHECK_OUT_EDIT_MODAL'
    } else {
      this.model.id = 0;
      this.title = 'SALE_ORDER_CHECK_OUT_ADD_MODAL';
    }
